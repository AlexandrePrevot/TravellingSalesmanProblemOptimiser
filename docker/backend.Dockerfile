...
RUN mkdir -p cmake/build && cd cmake/build && \
    cmake ../.. -DgRPC_INSTALL=ON -DgRPC_BUILD_TESTS=OFF -DCMAKE_INSTALL_PREFIX=/opt/grpc_install && \
...
RUN protoc --proto_path=messages/ --cpp_out=backend/generated --grpc_out=backend/generated/ --plugin=protoc-gen-grpc=/opt/grpc_install/bin/grpc_cpp_plugin messages/request.proto
